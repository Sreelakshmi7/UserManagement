...
router.post('/create', passport.authenticate('jwt', { session: false }), adminRoleMiddleware, validate(createVegetableValidation()), async (req, res) => {
...
router.get('/get/:vegetable_id', passport.authenticate('jwt', { session: false }), adminRoleMiddleware, validate(getVegetableValidation()), async (req, res) => {
    getVegetableById(req, res);
});

// Admin route to update vegetable by ID
router.patch('/update/:vegetable_id', passport.authenticate('jwt', { session: false }), adminRoleMiddleware, validate(updateVegetableValidation()), async (req, res) => {
    updateVegetableById(req, res);
});

// Admin route to delete vegetable by ID
router.delete('/delete/:vegetable_id', passport.authenticate('jwt', { session: false }), adminRoleMiddleware, validate(deleteVegetableValidation()), async (req, res) => {
